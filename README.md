...
- **Convergence Panel** — Gaussian and xtb structure optimization logs display a collapsible panel (📈 Convergence) showing energy, force (max + RMS), and displacement (max + RMS) curves versus optimization step, mirroring GaussView's convergence plot; includes threshold lines (dashed amber), a vertical resize handle to adjust panel height, and a close/reopen toggle. Force and displacement charts use a logarithmic Y axis — axis ticks show the log₁₀ values with a rotated `log₁₀` axis label — so the convergence-threshold lines stay readable even when early steps dwarf the final values (energy keeps a linear axis since it may be negative). Energies are read from `SCF Done` lines, or from the optimizer's `Energy= ... NIter=` line in `opt=external` runs (e.g. Gaussian + xTB/XO combined jobs, which print no `SCF Done`). For xtb trajectories, the gradient norm (`gnorm`) from each step's comment line is plotted as the RMS Force curve

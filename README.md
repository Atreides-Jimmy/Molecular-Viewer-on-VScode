...
- **Import Structure** — Click the Import Structure toolbar button (next to Add/Delete/Replace Atom) to load a second structure from any supported file (XYZ, GJF, MOL2, PDB, log/out trajectories, crystal formats, …) into the current view. Multi-frame files (optimization logs) open a frame picker; crystal files (CIF/VASP/Cube/VESTA) contribute their unit-cell atoms as a molecule. The imported structure is placed beside the existing one with a bounding-sphere separation test (both structures padded by 2.5 Å + a 2 Å gap), so it never overlaps; importing into an empty view centers the structure at the origin. The import can be repeated any number of times with the same or different files — each copy is stacked further along +X. Bonds merge smartly: if neither side has explicit bonds, connectivity is re-detected for the combined structure; if either side has authored bonds (MOL2/PDB/GJF connect), existing bonds are preserved and only the imported part's bonds are added. Each import is one undo step, the camera zooms out to fit, and saving exports the combined structure

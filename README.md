...
SMILES coverage and limits — the generator reads the OpenSMILES organic subset plus the common extensions: branches, `%nn` ring closures, aromaticity, explicit bond symbols, dot-disconnected fragments, isotopes, charges, and `@`/`@@` tetrahedral configuration. Tetrahedral chirality is honoured; a double bond written without `/` or `\` defaults to **trans**; the conformation comes from local geometric rules rather than a conformer search, so a long flexible chain may fold tightly; isotopes are parsed but do not influence geometry; organometallic geometry is not special-cased.

...
function getBondOrder(el1, el2, dist) {
...
function refineBondOrders(bondMap, atoms) {
    const maxIter = 10;
    for (let iter = 0; iter < maxIter; iter++) {
        let changed = false;
        const valencies = new Map();
...
function fixBondOrders(bondMap, atoms) {
    bondMap.forEach((neighbors, i) => {
        neighbors.forEach((order, j) => {
            if (i > j)
                return;
...
function detectBonds(atoms) {
    ...
    for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
            ...
            if (bo > 0) {
                if (!bondMap.has(i))
                    bondMap.set(i, new Map());
                if (!bondMap.has(j))
                    bondMap.set(j, new Map());

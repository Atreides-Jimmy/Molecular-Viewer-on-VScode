...
function parseMopacElement(token) {
    ...
    if (/^[A-Za-z]{1,2}$/.test(elem))
        return elem;
    return 'C';
}
function parseMopac(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!foundChargeMult) {
            ...
            if (cmMatch)
                charge = parseInt(cmMatch[1], 10);
            const msMatch = line.match(/(?:MULT|MS|MULTIPlicity)\s*=\s*(\d+)/i);
            if (msMatch)
                multiplicity = parseInt(msMatch[1], 10);
            if (cmMatch || msMatch) {
...
//# sourceMappingURL=mopacParser.js.map